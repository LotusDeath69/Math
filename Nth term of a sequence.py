...
def quadratic(x1, y1, x2, y2, x3, y3):
    ...
    a = checkDifference(checkDifference([y1, y2, y3], mode="quadratic"))[0] / 2 
    b = y2 - y1 - 3*a
    c = y1 - b - a 
...
def cubic(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5):
    if not checkDifference([x1, x2, x3, x4, x5]):
        return "Differences between x values are not consistent. \n Please try again."
    a = checkDifference(checkDifference(checkDifference([y1, y2, y3, y4, y5], mode="cubic"), mode="quadratic"))[0] / 6
    b = (checkDifference(checkDifference([y1, y2, y3, y4, y5], mode="cubic"), mode="quadratic")[0] - 12 * a) / 2
    c = (y2 - y1) - 7 * a - 3 * b
    d = y1 - a - b - c
    if not verify([x5, y5], [a, b, c, d], mode="cubic"):
        print("Unable to verify")
    return f"y = {a}x^3 + {b}x^2 + {c}x + {d}"

# print(linear(1, 4, 2, 6, 3, 8))
# print(quadratic(1, 4, 2, 6, 3, 8))
